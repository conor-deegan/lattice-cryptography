...
This distribution still only outputs $-1$, $0$, or $1$, but it is not uniform. It is centered around zero. This sort of distribution can be helpful, for example, we may prefer that errors should be close to zero, and larger errors should be less likely.
...
The values near zero are more likely than the values near $-2$ or $2$. This is the basic idea behind a centered binomial distribution. In cryptographic specifications, you may see notation like:

$$
x \leftarrow \mathrm{CBD}_\eta
$$

Read this as "sample $x$ from a centered binomial distribution with parameter $\eta$". The parameter $\eta$ controls how many random bits are summed on each side. The output range is from $-\eta$ to $\eta$.
