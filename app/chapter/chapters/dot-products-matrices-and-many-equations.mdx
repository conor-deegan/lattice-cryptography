...
has 2 columns. So it can be multiplied by a vector of length 2:
...
In general, an $(n \times m)$ matrix can be multiplied by an $(m \times t)$ matrix, and the result is an $(n \times t)$ matrix.

A length-$m$ column vector is an $(m \times 1)$ matrix, so a $(3 \times 2)$ matrix times a length-2 column vector gives a $(3 \times 1)$ column vector:
...
$$
As = \begin{bmatrix} \langle A_1, s \rangle \\\\ \langle A_2, s \rangle \\\\ \vdots \\\\ \langle A_m, s \rangle \end{bmatrix}
$$
...
Which is "matrix $A$ times secret vector $s$ equals vector $t$".
